package official.core.context_dispatchers
...
fun main() = runBlocking<Unit> {
    //sampleStart
    launch(Dispatchers.Unconfined) { // not confined -- will work with me.ztiany.tools.main thread
    ...
    launch { // context of the parent, me.ztiany.tools.main runBlocking coroutine
        println("me.ztiany.tools.main runBlocking: I'm working in thread ${Thread.currentThread().name}")
        delay(1000)
        println("me.ztiany.tools.main runBlocking: After delay in thread ${Thread.currentThread().name}")
    }
//sampleEnd
}