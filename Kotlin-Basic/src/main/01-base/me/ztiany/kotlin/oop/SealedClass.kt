...
private data class Const(val number: Double) : SealedClass()
private data class Sum(val e1: SealedClass, val e2: SealedClass) : SealedClass()
private object NotANumber : SealedClass()

private fun eval(aClass: SealedClass): Double =
    when (aClass) {
        is Const -> aClass.number
        is Sum -> eval(aClass.e1) + eval(aClass.e2)
