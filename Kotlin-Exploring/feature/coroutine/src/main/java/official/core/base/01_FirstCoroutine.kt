...
fun main() {
    GlobalScope.launch {
        ...
        println("CommonPool thread:" + Thread.currentThread())//Thread[ForkJoinPool.commonPool-worker-1,5,me.ztiany.tools.main]
    }
    println("Hello,") // me.ztiany.tools.main thread continues while coroutine is delayed
    Thread.sleep(2000L) // block me.ztiany.tools.main thread for 2 seconds to keep JVM alive，因为协程类似守护线程，主线程退出协程就自动退出了
}
