...
当调用 launch { …… } 时不传参数，它从启动了它的 CoroutineScope 中承袭了上下文（以及调度器）。在这个案例中，它从 me.ztiany.tools.main 线程中的 runBlocking 主协程承袭了上下文。
...
fun main() = runBlocking<Unit> {
    //sampleStart
    launch {
        // context of the parent, me.ztiany.tools.main runBlocking coroutine
        println("me.ztiany.tools.main runBlocking      : I'm working in thread ${Thread.currentThread().name}")
    }
    launch(Dispatchers.Unconfined) {
        // not confined -- will work with me.ztiany.tools.main thread
